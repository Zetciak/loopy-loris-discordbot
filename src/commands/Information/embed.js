...
const roadmap = require('../../embeds/roadmap.js');
const utility = require('../../embeds/utility.js');
const insiderinfo = require('../../embeds/insiderinfo.js');
const intern = require('../../embeds/intern.js');
const influencer = require('../../embeds/influencer.js');
const wallets = require('../../embeds/wallets.js');
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option
				...
				.addChoice('roadmap', 'roadmap')
				.addChoice('utility', 'utility')
				.addChoice('insiderinfo', 'insiderinfo')
				.addChoice('intern', 'intern')
				.addChoice('influencer', 'influencer')
				.addChoice('wallets', 'wallets')
		),
	async execute(interaction, client) {
		if (
			...
			if (interaction.options.getString('embedname') === 'verifier') {
				...
				interaction.options.getString('embedname') === 'roadmap'
			) {
				roadmap.sendEmbed(interaction, client);
				send = true;
			} else if (
				interaction.options.getString('embedname') === 'utility'
			) {
				utility.sendEmbed(interaction, client);
				send = true;
			} else if (
				interaction.options.getString('embedname') === 'insiderinfo'
			) {
				insiderinfo.sendEmbed(interaction, client);
				send = true;
			} else if (
				interaction.options.getString('embedname') === 'intern'
			) {
				intern.sendEmbed(interaction, client);
				send = true;
			} else if (
				interaction.options.getString('embedname') === 'influencer'
			) {
				influencer.sendEmbed(interaction, client);
				send = true;
			} else if (
				interaction.options.getString('embedname') === 'wallets'
			) {
				wallets.sendEmbed(interaction, client);
				send = true;
			}
